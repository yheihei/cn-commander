...
- [ ] **task-1-1**: Phaser3プロジェクト初期セットアップ（1280x720解像度、30FPS設定）
- [ ] **task-1-2**: ゲームステート管理システム（メニュー、ゲーム中、ポーズ等）
- [ ] **task-1-3**: シーン管理システム（タイトル、ステージ選択、バトル、結果画面）
- [ ] **task-1-4**: 16x16ドット絵アセット読み込みシステム

### 【エピック2】マップ・地形システム - 256x256マップ、地形効果、移動コスト
**優先度**: 高

- [ ] **task-2-1**: タイルマップシステム実装（256x256グリッド）
- [ ] **task-2-2**: 地形タイプ定義（平地、森林、山地）と描画
- [ ] **task-2-3**: 地形効果システム（移動コスト、防御補正、攻撃補正、視界補正）
- [ ] **task-2-4**: カメラシステム（スクロール、ズーム）

### 【エピック3】キャラクター・軍団システム - 職業、能力値、軍団編成
**優先度**: 高

- [ ] **task-3-1**: キャラクター基本システム（HP、攻撃力、防御力、速さ、移動速度、視界）
- [ ] **task-3-2**: 職業システム（風忍、鉄忍、影忍、薬忍）とクラススキル
- [ ] **task-3-3**: 軍団システム（指揮官1名＋一般兵3名）
- [ ] **task-3-4**: 軍団編成UI（待機兵士リスト、編成画面、アイテム装備）

### 【エピック4】移動システム - 経路指定、移動モード、移動速度計算
**優先度**: 高

- [ ] **task-4-1**: 移動指示システム（最大4地点の経路指定）
- [ ] **task-4-2**: 移動モード（通常移動、戦闘移動、待機）
- [ ] **task-4-3**: 移動速度計算（軍団平均速度、地形コスト）
- [ ] **task-4-4**: 移動アニメーション・軌跡表示
