...
- [ ] **task-7-5**: 拠点UI（拠点ActionMenu、施設サブメニュー、軍団編成画面）
  ...
  - [ ] **task-7-5-3**: 軍団編成画面統括実装（task-3-4-1、3-4-2、3-4-3の統合）
  - [ ] **task-7-5-4**: 医療施設サブメニュー実装
...
- [ ] **task-9-3**: 持物管理システム
  - [ ] **task-9-3-1**: 持物管理UI基盤実装（ActionMenu統合、ItemInventoryUI作成、ボタン配置）
  - [ ] **task-9-3-2**: 消耗品使用機能実装（兵糧丸HP回復、薬忍スキル連携）
  - [ ] **task-9-3-3**: 武器装備変更機能実装（装備/装備解除、装備中表示）
  - [ ] **task-9-3-4**: アイテム譲渡機能（UI表示のみ、将来実装用）
- [ ] **task-9-4**: 装備管理UI（task-3-4-2のアイテム選択UIと連携）
