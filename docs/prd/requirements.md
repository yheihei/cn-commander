...
##### 移動指示の操作フロー
1. **軍団選択**: 移動させたい軍団の指揮官を左クリック。指揮官はマークを付けてわかるようにしておく
2. **行動選択メニュー**: 
   - 指揮官選択時、行動選択UIが表示される
   - 現在は「移動」のみ（今後、他の行動を追加予定）
3. **移動モード選択**: 
   - 「移動」を選択すると、移動モード選択UIが表示
   - 「通常移動」または「戦闘移動」を選択
4. モード選択後、「移動経路を4地点まで選択してください」というメッセージを表示。他のところをクリックしたら表示終了
5. **経路指定**: 
   - 2秒待ってから経路選択モード開始
   - 最大4地点を順番にクリックして経路を設定
   - 選択した地点には「✕」マークが表示される
   - 各地点は直線で結ばれる
   - 地形を考慮した最短経路ではなく、指定通りの経路を移動
6. **移動開始**: 
   - 4つ目の経路が押されて2s経ったら、「✕」マークを消去し、軍団を指定経路に沿って移動開始させる
...
- **アイテム補給**: 拠点内の軍団にアイテムを配布（ステージ中も可能）
- **入院**: 軍団を2分間で全回復（500両）
- **駐留**: 味方軍団の待機場所として機能
...
#### 6.1.5 拠点内システム
- **駐留可能条件**: 味方拠点と隣接する味方軍団
- **駐留中の機能**:
  - アイテム補給: 倉庫から装備を受け取り（ステージ中の補給が可能）
  - 入院: 500両で2分間の全回復（入院中は出撃不可）
  - 防御: 拠点が攻撃された際に自動迎撃（通常の戦闘計算式を適用）
- **出撃**: 「出撃」コマンドで拠点周囲2マスに配置
