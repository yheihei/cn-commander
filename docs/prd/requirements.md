...
#### 6.2.1 資金管理システム概要
資金管理システムは、プレイヤーの経済活動を統括する中核システムである。全ての収入と支出を一元管理し、リアルタイムで資金状況を更新する。

##### 資金管理クラス（FundManager）の責務
- **資金の保持と更新**: 現在の所持金を管理
- **収入の自動計算**: 毎分の定期収入を自動的に加算
- **支出の処理**: 各種支出（生産、治療等）の即時反映
- **収支履歴の記録**: 直近の収支履歴を保持（デバッグ用）
- **資金不足チェック**: 支出前の残高確認機能
- **UI更新通知**: 資金変動時のUI自動更新
...
- **収入計算式**:
  ```
  総収入/分 = 本拠地収入 + Σ(占領拠点収入)
  ```
- **収入タイミング**: リアルタイムで1分ごとに自動加算
...
- **支出項目**:
  - アイテム生産費: 各アイテムごとに設定
    ...
    - 手裏剣: 200両
    - 兵糧丸: 50両
  - 兵士雇用: ステージ間でのみ可能（価格は兵士クラスにより変動）
  - 医療費用: 500両（軍団全回復）
- **支出タイミング**: 
  - 生産: 生産完了時に支払い（資金不足時は生産停止）
  - 治療: 治療開始時に即時支払い（前払い制）
  - 雇用: 雇用確定時に即時支払い
...
- **表示内容**:
  ...
  - 直近の支出履歴（最新5件）:
    ...
    - 「-500両 第2軍団治療」
  - 生産予約による予定支出額
- **表示タイミング**: 味方拠点を選択した時のみ

##### UIデザイン指針
- **視認性**: 数値は大きく、コントラストの高い色で表示
- **アニメーション**: 資金増減時に数値が一時的に光る演出
- **警告表示**: 資金不足時は赤色で警告表示
- **桁区切り**: 3桁ごとにカンマで区切り（例: 12,345両）
