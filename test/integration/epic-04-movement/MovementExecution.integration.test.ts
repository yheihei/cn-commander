...
describe("[エピック4] Movement Execution Integration Tests", () => {
  ...
  describe("移動開始処理", () => {
    ...
    test("×マークが経路選択完了後に消去される", () => {
      ...
      const handleLeftClick = (inputHandler as any).handleLeftClick.bind(inputHandler);
      
      // 指揮官の位置をクリック（軍団選択）
      handleLeftClick(160, 160);
  ...
  describe("エラーケース", () => {
    test("経路点なしで移動開始した場合", () => {
      ...
      // 移動が開始されないことを確認
      expect(army.isMoving()).toBe(false);
      expect(commandSystem.getCommand(army.getId())).toBeUndefined();
    });
