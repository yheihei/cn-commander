...
describe('[エピック10] 生産開始フロー統合テスト', () => {
  ...
  describe('キューへの追加（0/指定数形式）', () => {
    test('アイテムをキューに追加できる（初期状態は0/指定数）', () => {
      ...
      // 忍者刀20個の生産を追加
      const lineIndex = productionManager.addToQueue(baseId, ProductionItemType.NINJA_SWORD, 20);
    ...
    test('複数のアイテムを異なるラインに追加できる', () => {
      ...
      // 1つ目：忍者刀10個
      const line1 = productionManager.addToQueue(baseId, ProductionItemType.NINJA_SWORD, 10);
      expect(line1).toBe(0);

      // 2つ目：手裏剣5個
      const line2 = productionManager.addToQueue(baseId, ProductionItemType.SHURIKEN, 5);
      expect(line2).toBe(1);

      // 3つ目：兵糧丸99個（最大数）
      const line3 = productionManager.addToQueue(baseId, ProductionItemType.FOOD_PILL, 99);
      expect(line3).toBe(2);
    ...
    test('6ライン全て使用した場合は追加できない', () => {
      ...
      // 6つのラインを全て使用
      for (let i = 0; i < 6; i++) {
        const result = productionManager.addToQueue(baseId, ProductionItemType.NINJA_SWORD, 1);
        expect(result).toBe(i);
      }

      // 7つ目は追加できない
      const result = productionManager.addToQueue(baseId, ProductionItemType.SHURIKEN, 1);
    ...
    test('数量が範囲外の場合は追加できない', () => {
      ...
      // 0個は無効
      let result = productionManager.addToQueue(baseId, ProductionItemType.NINJA_SWORD, 0);
      expect(result).toBeNull();

      // 100個は無効（最大99）
      result = productionManager.addToQueue(baseId, ProductionItemType.NINJA_SWORD, 100);
      expect(result).toBeNull();

      // 負数は無効
      result = productionManager.addToQueue(baseId, ProductionItemType.NINJA_SWORD, -1);
      expect(result).toBeNull();
    });
  });
