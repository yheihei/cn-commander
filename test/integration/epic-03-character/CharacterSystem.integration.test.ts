...
describe('[エピック3] Character and Army System Integration Tests', () => {
  ...
  describe('キャラクター基本システム', () => {
    ...
    test('能力値が範囲内に制限される', () => {
      ...
      const character = CharacterFactory.createCharacter(scene, 0, 0, 'iron', 'テスト', customStats);
  ...
  describe('軍団システム', () => {
    ...
    test('軍団の平均移動速度が正しく計算される', () => {
      ...
      if (army) {
        ...
        const expectedSpeed = characters.reduce((sum, char) => 
          sum + char.getStats().moveSpeed, 0
        ) / characters.length;
