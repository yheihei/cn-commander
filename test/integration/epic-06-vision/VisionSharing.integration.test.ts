...
describe('[エピック6] 視界共有システム統合テスト', () => {
  ...
  describe('勢力視界共有', () => {
    test('同一勢力の複数軍団で視界を共有する', () => {
      // プレイヤー軍団を2つ作成
      const playerArmy1 = ArmyFactory.createPlayerArmyAtGrid(scene, armyManager, 12, 12);
      ...
      // 敵軍団を作成（軍団2の視界内）
      const enemyArmy = ArmyFactory.createEnemyArmyAtGrid(scene, armyManager, 40, 40);
      ...
      // 共有視界データを取得
      const sharedVision = visionSystem.getSharedVisionForFaction('player', [
        playerArmy1,
        playerArmy2,
      ]);
      ...
      // 勢力視界では敵が見える（共有）
      expect(visionSystem.isVisibleByFaction(enemyArmy, 'player', [playerArmy1, playerArmy2])).toBe(
        true,
      );
    });
    ...
    test('視界共有による発見メカニクス', () => {
      ...
      const army2 = ArmyFactory.createPlayerArmyAtGrid(scene, armyManager, 50, 50);
      if (army2) playerArmies.push(army2);

      // 敵軍団を作成（偵察隊長の視界内）
      const enemyArmy = ArmyFactory.createEnemyArmyAtGrid(scene, armyManager, 11, 11);
    ...
    test('視界キャッシュの動作確認', async () => {
      const playerArmy = ArmyFactory.createPlayerArmyAtGrid(scene, armyManager, 18, 18);
      ...
      // 初回の視界計算
      const vision1 = visionSystem.getSharedVisionForFaction('player', [playerArmy]);

      // キャッシュが使われることを確認（100ms以内）
      const vision2 = visionSystem.getSharedVisionForFaction('player', [playerArmy]);
      ...
      // 新しい計算が行われることを確認
      const vision3 = visionSystem.getSharedVisionForFaction('player', [playerArmy]);
    ...
    test('待機モードでの視界共有', () => {
      // 通常移動中の軍団
      const movingArmy = ArmyFactory.createPlayerArmyAtGrid(scene, armyManager, 10, 10);

      // 待機モード中の軍団（視界+1）
      const waitingArmy = ArmyFactory.createPlayerArmyAtGrid(scene, armyManager, 30, 30);
    ...
    test('最も近い観測者の判定', () => {
      ...
      const a1 = ArmyFactory.createPlayerArmyAtGrid(scene, armyManager, 6, 6);
      ...
      const a3 = ArmyFactory.createPlayerArmyAtGrid(scene, armyManager, 12, 12);
      if (a3) playerArmies.push(a3);

      // 敵軍団（軍団3に最も近い）
      const enemyArmy = ArmyFactory.createEnemyArmyAtGrid(scene, armyManager, 15, 15);
      ...
      // 発見チェック
      discoverySystem.checkFactionDiscovery('player', playerArmies, [enemyArmy]);
  ...
  describe('パフォーマンステスト', () => {
    test('多数の軍団での視界共有処理', () => {
      ...
      // 視界共有計算
      const sharedVision = visionSystem.getSharedVisionForFaction('player', playerArmies);
