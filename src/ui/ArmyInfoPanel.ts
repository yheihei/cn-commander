...
export class ArmyInfoPanel extends Phaser.GameObjects.Container {
  ...
  private createMemberInfo(character: Character, yOffset: number, isCommander: boolean): Phaser.GameObjects.Container {
    ...
    // 名前と職業
    const nameText = this.scene.add.text(5, 5, 
    ...
    if (items.length > 0) {
      const itemText = this.scene.add.text(5, 52, 
